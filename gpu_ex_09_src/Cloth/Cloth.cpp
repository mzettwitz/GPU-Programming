...
void updateCloth(float3* newPos, float3* oldPos, float3* impacts, float3* velocity,
	float deltaTime, float stepsize, float3* normals);
...
void ClothSim::update(GLfloat deltaTime)
{
	...
	updateCloth((float3*)newPos, (float3*)oldPos, (float3*)devPtrImpact, (float3*)devPtrVelocity, deltaTime, stepSize, (float3*)normals);
