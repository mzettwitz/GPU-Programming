...
void generateGeometryVertexBuffer()
{
	...
	glBufferData(GL_ARRAY_BUFFER,positions.size() *sizeof(Vector),positions.data(),GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER,0);

	// TODO: Erzeugen eines Vertex Buffer Objects für die Normalen.
	glGenBuffers(1, &normalBufferHandle);
	glBindBuffer(GL_ARRAY_BUFFER,normalBufferHandle);
	glBufferData(GL_ARRAY_BUFFER,normals.size() *sizeof(Vector),normals.data(),GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER,0);
	...
	glBindBuffer(GL_ARRAY_BUFFER, positionBufferHandle);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vector), (void*)0);
	glBindBuffer(GL_ARRAY_BUFFER, normalBufferHandle);
	glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(Vector), (void*)0);
